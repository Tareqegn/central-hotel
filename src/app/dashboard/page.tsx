...
export default function ManagerDashboard() {
  ...
  const columns = [
    ...
    { title: 'In Progress', statusKey: 'In Progress', color: 'border-blue-500/30 text-blue-400 bg-blue-500/10' },
    { title: 'On the Way', statusKey: 'On the Way', color: 'border-purple-500/30 text-purple-400 bg-purple-500/10' },
    { title: 'Completed', statusKey: 'Completed', color: 'border-emerald-500/30 text-emerald-400 bg-emerald-500/10' },
  ];

  return (
    <div className="min-h-screen bg-[#070709] text-neutral-100 p-4 sm:p-8 font-sans relative overflow-x-hidden selection:bg-amber-500 selection:text-black">
      
      {/* Ambient background lighting glows */}
      <div className="absolute top-0 left-1/4 w-[600px] h-[300px] bg-amber-500/[0.03] blur-[120px] pointer-events-none rounded-full" />
      <div className="absolute top-20 right-1/4 w-[600px] h-[300px] bg-blue-600/[0.03] blur-[120px] pointer-events-none rounded-full" />

      <div className="max-w-[1600px] mx-auto space-y-6 relative z-10">

        {/* Top Header Bar */}
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-[#121216]/80 backdrop-blur-xl border border-white/[0.08] p-5 rounded-3xl shadow-2xl gap-4">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-[#18181c] border border-amber-500/30 flex items-center justify-center p-2 shadow-inner">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto justify-between lg:justify-end">
            {/* View Switcher Tabs */}
            <div className="flex bg-[#0b0b0d] p-1 rounded-2xl border border-white/[0.08]">
              <button
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="bg-[#0b0b0d] text-xs font-mono text-neutral-200 px-3.5 py-2.5 rounded-xl border border-white/[0.08] focus:border-amber-400 focus:outline-none w-36 sm:w-44"
              />
              <select
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5">
          <div className="bg-[#121216]/60 backdrop-blur-xl border border-white/[0.08] p-4 rounded-2xl shadow-xl">
          ...
          <div className={`backdrop-blur-xl border p-4 rounded-2xl shadow-xl transition-all ${urgentCount > 0 ? 'bg-red-500/10 border-red-500/50 animate-pulse' : 'bg-[#121216]/60 border-white/[0.08]'}`}>
        ...
        {/* TAB 1: LIVE OPERATIONS (Kanban Kanban Kanban) */}
        {activeTab === 'operations' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 animate-fadeIn">
            {columns.map((col) => {
              ...
              return (
                <div key={col.statusKey} className="bg-[#121216]/70 backdrop-blur-xl rounded-3xl border border-white/[0.08] p-4 flex flex-col min-h-[600px] shadow-2xl">
                  ...
                  <div className="space-y-3.5 flex-1">
                    {colRequests.length === 0 ? (
                      <div className="h-44 flex items-center justify-center text-neutral-500 text-xs font-mono border border-dashed border-white/[0.05] rounded-2xl">
                        No requests
                      </div>
                    ) : (
                      colRequests.map((req) => {
                          ...
                          <div 
                            ...
                            className={`p-4 rounded-2xl border flex flex-col justify-between shadow-lg transition-all ${
                              isDelayed 
                                ...
                                : req.status === 'Pending' 
                                ? 'bg-[#18181d] border-amber-500/40' 
                                : 'bg-[#0b0b0d]/80 border-white/[0.06] hover:border-amber-500/30'
                            }`}
                          >
                            <div>
                              ...
                              <p className="text-[11px] text-neutral-300 mb-3 bg-[#121215]/80 p-2.5 rounded-xl border border-white/[0.04] leading-relaxed">
                                {req.note}
                              </p>
                            ...
                            <div className="grid grid-cols-2 gap-1.5 pt-3 border-t border-white/[0.06]">
        ...
        {activeTab === 'management' && (
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="bg-[#121216]/70 backdrop-blur-xl border border-white/[0.08] p-6 rounded-3xl shadow-2xl">
              ...
              <form onSubmit={handleAddStaff} className="grid grid-cols-1 sm:grid-cols-5 gap-3 mb-4">
                <input
                  ...
                  className="bg-[#0b0b0d] text-neutral-200 text-xs font-mono px-3.5 py-3 rounded-2xl border border-white/[0.08] focus:border-amber-400 focus:outline-none"
                />
                <select
                  value={newStaffDept}
                  onChange={(e) => setNewStaffDept(e.target.value)}
                  className="bg-[#0b0b0d] text-amber-400 text-xs font-mono px-3.5 py-3 rounded-2xl border border-amber-500/30 focus:outline-none capitalize cursor-pointer"
                >
                ...
                <input
                  ...
                  className="bg-[#0b0b0d] text-amber-400 text-xs font-mono px-3.5 py-3 rounded-2xl border border-white/[0.08] focus:border-amber-400 focus:outline-none"
                />
                <button
                  type="submit"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 max-h-48 overflow-y-auto pr-1">
                {dbStaffMembers.length === 0 ? (
                  ...
                  dbStaffMembers.map((staff, idx) => (
                    <div key={idx} className="flex items-center justify-between bg-[#0b0b0d] px-4 py-3 rounded-2xl border border-white/[0.05] shadow">
                      <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-[#121216]/70 backdrop-blur-xl border border-white/[0.08] p-6 rounded-3xl shadow-2xl flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between mb-4">
                  ...
                  <input
                    ...
                    className="w-full bg-[#0b0b0d] text-neutral-200 text-xs font-mono px-3.5 py-3 rounded-2xl border border-white/[0.08] focus:border-amber-400 focus:outline-none mb-4"
                  />
                </div>
                ...
                <div className="space-y-2 max-h-32 overflow-y-auto">
                  {guestProfiles.length === 0 ? (
                    ...
                    guestProfiles.map((p) => (
                      <div key={p.id} className="flex items-center justify-between bg-[#0b0b0d] px-3.5 py-2.5 rounded-2xl border border-white/[0.05] text-xs">
              ...
              <div className="bg-[#121216]/70 backdrop-blur-xl border border-white/[0.08] p-6 rounded-3xl shadow-2xl flex flex-col justify-between">
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-[#0b0b0d] text-neutral-200 text-xs font-mono p-4 rounded-2xl border border-white/[0.08] focus:border-amber-400 focus:outline-none h-36 resize-none leading-relaxed"
                  />
                </div>
              </div>
            ...
            <div className="bg-[#121216]/70 backdrop-blur-xl border border-white/[0.08] p-6 rounded-3xl shadow-2xl">
              <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                ...
                <div className="flex items-center gap-2.5 flex-wrap">
                  {announcementTarget === 'staff' ? (
                    <>
                      <select
                        ...
                        className="bg-[#0b0b0d] text-blue-400 font-mono text-xs px-3.5 py-2 rounded-xl border border-blue-500/30 focus:outline-none capitalize cursor-pointer"
                      >
                      ...
                      <select
                        ...
                        className="bg-[#0b0b0d] text-emerald-400 font-mono text-xs px-3.5 py-2 rounded-xl border border-emerald-500/30 focus:outline-none cursor-pointer"
                      >
                        <option value="all">All Staff</option>
                        {filteredStaffNames.map((name) => <option key={name} value={name}>{name}</option>)}
                      </select>
                    </>
                  ) : (
                    <select
                      ...
                      className="bg-[#0b0b0d] text-amber-400 font-mono text-xs px-3.5 py-2 rounded-xl border border-amber-500/30 focus:outline-none cursor-pointer"
                    >
                  ...
                  <div className="flex bg-[#0b0b0d] p-1 rounded-xl border border-white/[0.08]">
              ...
              <form onSubmit={handlePublishAnnouncement} className="flex gap-3 mb-4">
                <input
                  ...
                  className="flex-1 bg-[#0b0b0d] text-neutral-200 text-xs font-mono px-4 py-3 rounded-2xl border border-white/[0.08] focus:border-amber-400 focus:outline-none"
                />
              ...
              <div className="space-y-2 max-h-36 overflow-y-auto">
                {announcements.length === 0 ? (
                  ...
                  announcements.map((ann) => (
                    <div key={ann.id} className="flex items-center justify-between bg-[#0b0b0d] px-4 py-2.5 rounded-2xl border border-white/[0.05]">
